...
/**
 * Markdown内にJSONを安全に埋め込むため、見やすい形に整形する。
 *
...
/**
 * 指定サイトの reports/{site_id}/data からJSONを読み込み、
 * reports/{site_id}/ai-harness.md を生成する。
 *
 * @param site_id レポート対象サイトID
 */
async function generate_ai_harness(site_id: string): Promise<void> {
  const site_report_dir = path.join(reports_dir, site_id);
  const data_dir = path.join(site_report_dir, 'data');

  const smoke_summary_path = path.join(data_dir, 'smoke-summary.json');
  const navigation_summary_path = path.join(data_dir, 'navigation-summary.json');
  const ai_harness_path = path.join(site_report_dir, 'ai-harness.md');

  const smoke_summary = await read_json<SmokeSummary>(smoke_summary_path);
  const navigation_summary = await read_json<NavigationSummary>(
...
/**
 * reports/配下から、必要なJSONを持っているサイトIDだけを取得する。
 *
...
async function find_report_site_ids(): Promise<string[]> {
  ...
  for (const entry of entries) {
    ...
    const data_dir = path.join(reports_dir, site_id, 'data');
